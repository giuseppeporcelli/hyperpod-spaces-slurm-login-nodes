...
7. Use `gosu` to drop privileges to the target user and launch the IDE:
   - JupyterLab: runs the SageMaker runtime script with `ServerApp.root_dir` set to the user's home
   - Code Editor: runs `sagemaker-code-editor` on port 8888 with the user's home as the default folder
...
Workspace Pod starts with custom image
  ...
  └─ gosu drops to target user → launches JupyterLab or Code Editor
```
